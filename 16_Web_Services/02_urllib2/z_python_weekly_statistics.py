...
class Collector(object):
    ...
    def report(self):
        hosts, total = 0, 0
        for k in sorted(
            self.data["hosts"].keys(), key=lambda x: self.data["hosts"][x], reverse=True
        ):
            hosts += 1
            total += int(self.data["hosts"][k])
            # print(k, self.data['hosts'][k])
            print(
                '<li><a href="http://{0}/">{0}</a> {1}</li>'.format(
                    k, self.data["hosts"][k]
                )
            )
        print(
            "A total of {} links to {} hosts in {} issues.".format(
                total, hosts, self.data["issue"]
            )
        )
